 ...
 */
export const createRelation = ({ id: providedId, fromEntity, toEntity, position, fromSpace, toSpace, fromVersion, toVersion, type, entityId: providedEntityId, entityName, entityDescription, entityCover, entityValues, entityRelations, entityTypes, }) => {
    if (providedId)
    ...
    if (fromSpace)
        assertValid(fromSpace, '`fromSpace` in `createRelation`');
    if (toSpace)
        assertValid(toSpace, '`toSpace` in `createRelation`');
    if (fromVersion)
        assertValid(fromVersion, '`fromVersion` in `createRelation`');
    if (toVersion)
        assertValid(toVersion, '`toVersion` in `createRelation`');
    if (type)
...
//# sourceMappingURL=create-relation.js.map
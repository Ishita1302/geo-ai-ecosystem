import { updateEntity as grcUpdateEntity, languages, } from '@geoprotocol/grc-20';
 ...
 */
const toGrcUnsetLanguage = (language) => {
    ...
    if (language instanceof Uint8Array) {
        return { type: 'specific', language: language };
    }
 ...
 */
export const updateEntity = ({ id, name, description, values, unset }) => {
    ...
    for (const valueEntry of values ?? []) {
        ...
        if (valueEntry.type === 'text') {
            ...
            const exhaustiveCheck = valueEntry;
            throw new Error(`Unsupported value type: ${exhaustiveCheck.type}`);
        }
    }
    const unsetValues = (unset ?? []).map(unsetEntry => ({
        property: toGrcId(unsetEntry.property),
        language: toGrcUnsetLanguage(unsetEntry.language),
    }));
    const op = grcUpdateEntity({
        id: toGrcId(id),
        set: newValues,
        unset: unsetValues,
    });

...
describe('createEntity', () => {
    ...
    it('throws an error if a unit id in integer value is invalid', () => {
        const propertyId = Id('295c8bc61ae342cbb2a65b61080906ff');
        expect(() => createEntity({ values: [{ property: propertyId, type: 'integer', value: 42n, unit: 'invalid-unit' }] })).toThrow('Invalid id: "invalid-unit" for `unit` in `values` in `createEntity`');
    });
    ...
    it('throws error for unsafe integer in integer value', () => {
        ...
        })).toThrow(`Value ${unsafeInteger} is outside safe integer range for \`integer\` value in \`createEntity\`. Use bigint for large integers.`);
    });
});
